...
The workspace test runner's load hook scans a spec's whole source for `jest.mock()` (and `jest.unmock()`) declarations before any of its imports are linked, so placement never affects execution: the mocks register first wherever they sit. Writing them _before_ the imports therefore buys nothing and pushes the real module dependencies below a wall of mock boilerplate.

This rule is a style convention: every mock-registration call is written **after** all `import` statements. It is applied to every `*.spec.ts` file in the workspace.
