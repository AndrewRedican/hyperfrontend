...
Most state libraries let you model anything. This one models one thing well: the lifecycle of an async operation. Four booleans go in ([`inProgress`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-inProgress), [`success`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-success), [`fail`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-fail), [`halt`](https://www.hyperfrontend.dev/docs/libraries/state-machine/models/#api-State-prop-halt)) and named states come out, and the interesting ones are the combinations a lone isLoading flag throws away. Running after a failure is [`retrying`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-retrying), not another first attempt. Running after a success is [`restarting`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-restarting), which is the state a table is in while it refreshes with the old rows still on screen. Never run at all is [`notStarted`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-notStarted), which is usually a different screen from a spinner. Those distinctions come out of the same start, success, and fail dispatches you were going to write anyway.

There is no framework here and no third-party dependency: no provider to mount, no hook, no adapter. [`Store`](https://www.hyperfrontend.dev/docs/libraries/state-machine/store/#api-Store) is a subscribe and dispatch container over one reducer. [`AsyncOperation`](https://www.hyperfrontend.dev/docs/libraries/state-machine/async-operation/#api-AsyncOperation) wraps a promise-returning function and dispatches start, success, and fail around it so you can listen for events instead of polling flags. [`LifecycleAwareComponent`](https://www.hyperfrontend.dev/docs/libraries/state-machine/lifecycle-aware-component/#api-LifecycleAwareComponent) is a base class for anything with an init step, and it closes the usual startup race: a callback registered after the component is already ready fires immediately rather than waiting for a transition that has been and gone.
...
- **[Redux-inspired architecture](https://www.hyperfrontend.dev/docs/libraries/state-machine/store/)** - Central store with immutable state, action dispatching, and pure reducers
- **[Pre-built process reducer](https://www.hyperfrontend.dev/docs/libraries/state-machine/reducer/)** - Ready-to-use reducer for START/PAUSE/CANCEL/SUCCESS/FAIL action types
- **[Derived state computation](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/)** - Automatic computation of notStarted, done, paused, cancelled, retrying, restarting from core state
- **[Event-driven notifications](https://www.hyperfrontend.dev/docs/libraries/state-machine/events/)** - Observer pattern with event handlers triggered when specific derived states activate
- **[Async operation wrapper](https://www.hyperfrontend.dev/docs/libraries/state-machine/async-operation/)** - [`AsyncOperation`](https://www.hyperfrontend.dev/docs/libraries/state-machine/async-operation/#api-AsyncOperation) class that automatically dispatches actions for promise lifecycles
- **[Coordinated async processes](https://www.hyperfrontend.dev/docs/libraries/state-machine/coordinated-async-operation/)** - [`CoordinatedAsyncProcess`](https://www.hyperfrontend.dev/docs/libraries/state-machine/coordinated-async-operation/#api-CoordinatedAsyncProcess) for managing multiple async operations with startAll/cancelAll/pauseAll
- **[Lifecycle-aware components](https://www.hyperfrontend.dev/docs/libraries/state-machine/lifecycle-aware-component/)** - Abstract class with initializing/ready/starting/stopping/active state tracking and callbacks
...
isLoading plus error is two booleans and four combinations, and none of them record what happened last time. It cannot tell a first load from a refresh, so the spinner covers rows you could have kept on screen. It cannot tell a retry from a first attempt, so the error banner vanishes the moment you try again and nobody can see that a second attempt is running. Here those are [`restarting`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-restarting) and [`retrying`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-retrying), both falling out of the start and fail dispatches you were already making, and [`notStarted`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-notStarted) stays distinct from [`inProgress`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-inProgress), so an empty screen and a loading screen are different renders.

### Nothing to mount, nothing to adapt

There is no provider, no hook, no framework binding, no third-party package pulled in behind it. [`store.subscribe`](https://www.hyperfrontend.dev/docs/libraries/state-machine/store/#api-Store-prop-subscribe) returns its own unsubscribe, [`AsyncOperation`](https://www.hyperfrontend.dev/docs/libraries/state-machine/async-operation/#api-AsyncOperation) is a class you can hold in a closure, and both work the same in a React effect, a Svelte store, a worker, or a plain script. That matters most in the places a framework store cannot reach: a shared module, an SDK, an embed, a Node process.

### Init races handled by the base class, not by convention

Anything with a startup step has the same bug waiting in it: something subscribes after initialization finished and never hears about it. [`LifecycleAwareComponent`](https://www.hyperfrontend.dev/docs/libraries/state-machine/lifecycle-aware-component/#api-LifecycleAwareComponent) fires a freshly registered [`onReadyStatusChange`](https://www.hyperfrontend.dev/docs/libraries/state-machine/lifecycle-aware-component/#api-LifecycleAwareComponent) or [`onActiveStatusChange`](https://www.hyperfrontend.dev/docs/libraries/state-machine/lifecycle-aware-component/#api-LifecycleAwareComponent) callback right away if the component is already in that state, and its setters skip the notification entirely when a value has not changed, so subscribers get one call per real transition and never miss the one that already happened.

### Coordination that is honest about what it can do

[`CoordinatedAsyncProcess`](https://www.hyperfrontend.dev/docs/libraries/state-machine/coordinated-async-operation/#api-CoordinatedAsyncProcess) registers several processes and gives you `startAll()`, `pauseAll()`, and `cancelAll()`. Be clear on what cancel means: it marks the operations halted, it does not abort a promise already in flight. If your work needs a real abort, pass an `AbortSignal` into the process yourself and use this for the state around it.
...
The whole state is four booleans: [`inProgress`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-inProgress), [`success`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-success), [`fail`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-fail), and [`halt`](https://www.hyperfrontend.dev/docs/libraries/state-machine/models/#api-State-prop-halt). A [`Store`](https://www.hyperfrontend.dev/docs/libraries/state-machine/store/#api-Store) holds one copy of them, five action creators ([`start`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-start), [`pause`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-pause), [`cancel`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-cancel), [`success`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-success), [`fail`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-fail)) are the only things that move them, and [`START`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-START) is the handler that carries history forward: it sets [`inProgress`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-inProgress) and leaves whatever [`success`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-success) or [`fail`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/#api-fail) was already there. That is the whole trick, and everything else reads those four flags back.

Reading them back is what the selectors do. Ten of them take a state and return a boolean, and [`derivedState`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-derivedState) computes the set in a single call and hands back a named object to render from. [`notStarted`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-notStarted), [`inProgress`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-inProgress), [`done`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-done), [`successful`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-successful), [`failed`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-failed), [`halted`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-halted), [`paused`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-paused), and [`cancelled`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-cancelled) are the ones you would expect. The two that pay for the package are [`retrying`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-retrying), true while an attempt runs after a failure, so the error banner can stay up instead of blinking off, and [`restarting`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/#api-restarting), true while a refresh runs with a good result still on screen, so a table can keep its rows. A lone isLoading is true at both of those moments and at a first load, and says the same thing at all three.
...
Every folder is its own entry point, so an import can take just the surface you need. Reach for [`/selectors`](https://www.hyperfrontend.dev/docs/libraries/state-machine/selectors/) and [`/models`](https://www.hyperfrontend.dev/docs/libraries/state-machine/models/) when something else already owns the state and you only want to read and type it; [`/store`](https://www.hyperfrontend.dev/docs/libraries/state-machine/store/), [`/actions`](https://www.hyperfrontend.dev/docs/libraries/state-machine/actions/), [`/reducer`](https://www.hyperfrontend.dev/docs/libraries/state-machine/reducer/), and [`/state`](https://www.hyperfrontend.dev/docs/libraries/state-machine/state/) to drive the flags yourself; [`/events`](https://www.hyperfrontend.dev/docs/libraries/state-machine/events/) and [`/state-change`](https://www.hyperfrontend.dev/docs/libraries/state-machine/state-change/) for the edge-triggered dispatcher and the previous-versus-current pair it compares; and [`/async-operation`](https://www.hyperfrontend.dev/docs/libraries/state-machine/async-operation/), [`/coordinated-async-operation`](https://www.hyperfrontend.dev/docs/libraries/state-machine/coordinated-async-operation/), or [`/lifecycle-aware-component`](https://www.hyperfrontend.dev/docs/libraries/state-machine/lifecycle-aware-component/) when you would rather a class did the dispatching. The root entry re-exports all of it, which is the easier start and the one the bundler can still tree-shake.
...
**Global variable:** [`HyperfrontendStateMachine`](https://www.hyperfrontend.dev/docs/libraries/state-machine/)
