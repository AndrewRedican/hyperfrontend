 ...
 *
 * Tests the complete end-to-end flow of the Nexus protocol handshake,
 * including happy path, denial, cancellation, and close flows.
 *
 * The Nexus protocol uses a three-way handshake:
 * 1. Initiator sends REQUEST_CONNECTION
 * 2. Responder validates and sends ACCEPT_CONNECTION (or DENY_CONNECTION)
 * 3. Initiator confirms with OPEN_CONNECTION
 * 4. Both sides fire 'open' event
 */

import { createBroker } from '../broker'
import type { IChannelContract } from '../types'
import { createMockWindow, linkMockWindows, createContractPair, type MockWindow } from './test-utils'
