...
| `'unresponsive'` | The heartbeat watchdog trips under the default `onUnresponsive` policy.         | `missedBeats`, `lastBeatAt`, `displayMode` |
...
- Everything else (`forms`, `popups`, `modals`, `downloads`, `topNavigationByUserActivation`) is denied unless opted in.
