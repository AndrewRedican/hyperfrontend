...
Opening is asynchronous: `isOpen` stays `false` and sends queue until the wire handshake with the feature completes, flushing on the `open` event. If the feature never completes the handshake within `openTimeoutMs` (default 10 s), the shell tears the mount down and emits `error` with `reason: 'open-timeout'`. Every shell-originated `error` payload carries a `reason` discriminator: `'open-timeout'` as above (with `elapsedMs` and `displayMode`), `'open-failed'` when the display mode cannot produce a feature window at all (e.g. a blocked popup), `'target-lost'` (with `elapsedMs` and `displayMode`) when a window the shell opened stops being reachable before the handshake completes, and `'unresponsive'` (with `missedBeats` and `lastBeatAt`) when the heartbeat watchdog trips under the default `onUnresponsive` policy.
