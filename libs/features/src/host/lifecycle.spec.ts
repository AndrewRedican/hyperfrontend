...
function setup(config: { target?: Window | null; settings?: Record<string, unknown>; frame?: HTMLElement } = {}) {
  ...
  const mount = jest.fn((ctx: MountContext): MountResult => {
    context = ctx
    return { target: 'target' in config ? <Window | null>config.target : TARGET, frame: config.frame, cleanup }
  })
