...
Two things bound that walk, and both of them keep it offline. `$ref` resolves inside the document only: the root schema's [`definitions`](https://www.hyperfrontend.dev/docs/libraries/utils/json/#api-Schema-prop-definitions) are indexed up front and any other `#/...` pointer is walked against the root, so a schema that refers back to itself validates arbitrarily deep recursive data with no network or filesystem access (a `$ref` that resolves to nothing is skipped, and one that loops without consuming data is reported). And with [`safePatterns`](https://www.hyperfrontend.dev/docs/libraries/utils/json/#api-ValidateOptions-prop-safePatterns) on, every [`pattern`](https://www.hyperfrontend.dev/docs/libraries/utils/json/#api-Schema-prop-pattern) and [`patternProperties`](https://www.hyperfrontend.dev/docs/libraries/utils/json/#api-Schema-prop-patternProperties) regex goes to [`checkPatternSafety`](https://www.hyperfrontend.dev/docs/libraries/utils/json/#api-checkPatternSafety), or to a checker of your own, before `RegExp` is ever constructed: a user-supplied pattern can then come back as an ordinary validation error instead of being executed.
