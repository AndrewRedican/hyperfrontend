...
Browser-optimized implementations of the [`@hyperfrontend/string-utils`](https://www.hyperfrontend.dev/docs/libraries/utils/string/) encoding API, built on `TextEncoder` / `TextDecoder` and `atob` / `btoa`.
...
The built-in references (`btoa`, `atob`, `TextEncoder`, `TextDecoder`) are pulled from [`@hyperfrontend/immutable-api-utils`](https://www.hyperfrontend.dev/docs/libraries/utils/immutable-api/) so consumers cannot break the library by overwriting globals.
...
Use this entry point in browsers, Web Workers, Deno, Bun, and Cloudflare Workers: anywhere the Web Encoding APIs are available. For Node.js processes, import from [`@hyperfrontend/string-utils/node`](https://www.hyperfrontend.dev/docs/libraries/utils/string/node/) instead so the `Buffer`-based implementations are picked up by the bundler.
