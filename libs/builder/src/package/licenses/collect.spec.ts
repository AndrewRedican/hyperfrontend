...
describe('collectThirdPartyLicenses', () => {
  ...
  it('returns an empty array when externals is empty', () => {
    expect(collectThirdPartyLicenses(workspaceRoot, [])).toEqual([])
  })
  ...
  it('collects a license entry for a top-level dependency', () => {
    ...
    const result = collectThirdPartyLicenses(workspaceRoot, ['rollup'])
  ...
  it('resolves scoped packages under node_modules/@scope/name', () => {
    ...
    const result = collectThirdPartyLicenses(workspaceRoot, ['@rollup/plugin-json'])
    expect(result).toHaveLength(1)
    expect(result[0].name).toBe('@rollup/plugin-json')
  })

  it('drops dependencies whose package.json is not present on disk', () => {
    const result = collectThirdPartyLicenses(workspaceRoot, ['missing'])
  ...
  it('falls back to the package.json license when no LICENSE file exists', () => {
    ...
    expect(collectThirdPartyLicenses(workspaceRoot, ['foo'])[0].licenseType).toBe('Apache-2.0')
  })

  it('marks the license as Unknown when neither package.json nor LICENSE convey it', () => {
    const dep = join(nodeModules, 'foo')
    writePkg(dep, { name: 'foo' })
    expect(collectThirdPartyLicenses(workspaceRoot, ['foo'])[0].licenseType).toBe('Unknown')
  })
  ...
  it('detects MIT license from LICENSE file content', () => {
    ...
    expect(collectThirdPartyLicenses(workspaceRoot, ['foo'])[0].licenseType).toBe('MIT')
  })
  ...
  it.each([
    ...
    expect(collectThirdPartyLicenses(workspaceRoot, ['foo'])[0].licenseType).toBe(type)
  })
  ...
  it('falls back to package.json license when LICENSE content is unrecognized', () => {
    ...
    expect(collectThirdPartyLicenses(workspaceRoot, ['foo'])[0].licenseType).toBe('CUSTOM')
  })
  ...
  it('constructs a GitHub license URL when the repository is a GitHub URL', () => {
    ...
    writeFileSync(join(dep, 'LICENSE'), 'MIT License')
    const url = collectThirdPartyLicenses(workspaceRoot, ['foo'])[0].licenseUrl
  ...
  it('returns null licenseUrl when no LICENSE file is present', () => {
    ...
    expect(collectThirdPartyLicenses(workspaceRoot, ['foo'])[0].licenseUrl).toBeNull()
  })
  ...
  it('builds a GitLab license URL when the repository points at gitlab', () => {
    ...
    writeFileSync(join(dep, 'LICENSE'), 'MIT License')
    expect(collectThirdPartyLicenses(workspaceRoot, ['foo'])[0].licenseUrl).toBe('https://gitlab.com/x/y/-/blob/HEAD/LICENSE')
  })
  ...
  it('builds a Bitbucket license URL when the repository points at bitbucket', () => {
    ...
    writeFileSync(join(dep, 'LICENSE'), 'MIT License')
    expect(collectThirdPartyLicenses(workspaceRoot, ['foo'])[0].licenseUrl).toBe('https://bitbucket.org/x/y/src/HEAD/LICENSE')
  })
  ...
  it('returns entries sorted alphabetically by package name', () => {
    ...
    const result = collectThirdPartyLicenses(workspaceRoot, ['zeta', 'alpha', 'mike'])
