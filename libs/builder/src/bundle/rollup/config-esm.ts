import type { OutputOptions, Plugin, RollupLog, RollupOptions } from 'rollup'
import type { BuildContext, EntryPoint, EsmConfig } from '../../models'
import { join } from '@hyperfrontend/project-scope/core/path'
import { buildWorkspaceRoutes, createExternalizeBundledDepsPlugin } from '../dependencies/externalize-plugin'
import { createExternalFn } from '../externals/external-fn'
import { resolveExternals } from '../externals/resolve-externals'
import { createCommonJsPlugin, createJsonPlugin, createNodeResolvePlugin, createTypescriptPlugin } from './plugins'

const createEsmOutput = (outputPath: string, sourcemap: boolean): OutputOptions => ({
  file: join(outputPath, 'index.esm.js'),
  format: 'esm',
  sourcemap,
})

const onWarn = (warning: RollupLog, defaultHandler: (warning: RollupLog) => void): void => {
  if (warning.plugin === 'typescript' && warning.message?.includes('TS2307')) return
  if (warning.code === 'EMPTY_BUNDLE') return
  defaultHandler(warning)
}

/**
 * Builds the Rollup configuration for the ESM output of a single entry point.
 *
 * @param entry - Entry point to compile.
 * @param config - ESM-format configuration.
 * @param context - Resolved build context.
 * @returns Rollup configuration.
 *
 * @example Producing the rollup config for the root entry
 * ```typescript
 * const rollupConfig = createEsmEntryConfig(entry, { bundleWorkspaceDeps: false }, context)
 * ```
 */
export const createEsmEntryConfig = (entry: EntryPoint, config: EsmConfig, context: BuildContext): RollupOptions => {
  const entryOutputPath = entry.srcPath ? join(context.outputPath, entry.srcPath) : context.outputPath
  const sourcemap = config.sourcemap ?? true
  const bundleAllRequested = Boolean(config.bundleAllDeps)
  const useBundleAllDeps = bundleAllRequested && context.bundledDeps.length > 0
  const bundledDeps = useBundleAllDeps ? context.bundledDeps : []
  const workspaceRoutes = bundleAllRequested ? buildWorkspaceRoutes(context.workspaceBundledDeps) : []
  const workspaceBundledDepNames = bundleAllRequested ? workspaceRoutes.map((r) => r.packageName) : []
  const external = resolveExternals({
    packageJsonPath: join(context.projectRoot, 'package.json'),
    additional: [...context.external, ...(config.external ?? [])],
    isWorkspacePackage: context.isWorkspacePackage,
    bundleWorkspaceDeps: config.bundleWorkspaceDeps,
    bundledDeps,
    workspaceBundledDepNames,
  })

  const plugins: Plugin[] = []
  if (useBundleAllDeps || workspaceRoutes.length > 0) {
    plugins.push(
      createExternalizeBundledDepsPlugin({
        deps: bundledDeps,
        entryOutDir: entryOutputPath,
        format: 'esm',
        depsRoot: join(context.outputPath, '_dependencies'),
        workspaceRoutes,
      })
    )
  }
  plugins.push(
    createJsonPlugin(),
    createNodeResolvePlugin(),
    createCommonJsPlugin(),
    createTypescriptPlugin({
      tsConfigPath: context.tsConfigPath,
      projectRoot: context.projectRoot,
      outputPath: entryOutputPath,
      sourcemap,
      bundleWorkspaceDeps: config.bundleWorkspaceDeps,
      workspaceRoot: context.workspaceRoot,
    })
  )

  return {
    input: entry.inputFile,
    external: createExternalFn(external),
    onwarn: onWarn,
    plugins,
    output: createEsmOutput(entryOutputPath, sourcemap),
  }
}

/**
 * Builds the Rollup configuration for the ESM output of every supplied entry point.
 *
 * @param entries - Entry points to compile.
 * @param config - ESM-format configuration shared across the entries.
 * @param context - Resolved build context.
 * @returns One Rollup configuration per entry.
 *
 * @example Producing rollup configs for every discovered entry
 * ```typescript
 * const configs = createEsmConfig(discovery.entryPoints, { bundleWorkspaceDeps: false }, context)
 * ```
 */
export const createEsmConfig = (entries: EntryPoint[], config: EsmConfig, context: BuildContext): RollupOptions[] =>
  entries.map((entry) => createEsmEntryConfig(entry, config, context))
