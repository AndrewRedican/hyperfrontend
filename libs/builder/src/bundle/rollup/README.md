...
The actual Rollup configuration lives entirely inside the worker's `job-runner.ts`: plugin wiring (`json`, `node-resolve`, `commonjs`, `typescript`, `terser`), output options, and `onwarn` suppression. That file depends only on `rollup` and the `@rollup/plugin-*` packages, so it can bootstrap before any workspace package is built.
