...
describe('runPrePass', () => {
  ...
  it('forks one worker per job sequentially and aggregates the JSON reports', async () => {
    ...
    writeReport(records[0]!.reportPath, { outputSize: 100, peakHeapMB: 1.5, peakRssMB: 50, durationMs: 25 })
    records[0]!.child.emit('exit', 0)
    await tick()
    expect(records).toHaveLength(2)
    writeReport(records[1]!.reportPath, { outputSize: 200, peakHeapMB: 2.5, peakRssMB: 75, durationMs: 30 })
    records[1]!.child.emit('exit', 0)
    const results = await promise
    expect(results).toHaveLength(2)
    expect(results[0]).toMatchObject({ outputSize: 100, peakHeapMB: 1.5, peakRssMB: 50, durationMs: 25, job: jobs[0] })
    expect(results[1]).toMatchObject({ outputSize: 200, peakHeapMB: 2.5, peakRssMB: 75, durationMs: 30, job: jobs[1] })
  })
  ...
  it('rejects when the worker reports a spawn error', async () => {
    ...
    const promise = runPrePass([baseJob({ dep: 'rollup' })], { workerPath: '/abs/worker.cjs.js' })
    await tick()
    records[0]!.child.emit('error', new Error('ENOENT'))
  ...
  it('passes execPath and execArgv overrides to spawn', async () => {
    ...
    await tick()
    expect(records[0]!.execPath).toBe('/usr/bin/custom-node')
    expect(records[0]!.args.slice(0, 3)).toEqual(['--require', '@swc-node/register', '/abs/worker.cjs.js'])
    writeReport(records[0]!.reportPath, { outputSize: 1, peakHeapMB: 1, peakRssMB: 1, durationMs: 1 })
    records[0]!.child.emit('exit', 0)
    await promise
  })
  ...
  it('invokes the optional memory monitor before and after each job', async () => {
    ...
    await tick()
    writeReport(records[0]!.reportPath, { outputSize: 10, peakHeapMB: 1, peakRssMB: 1, durationMs: 1 })
    records[0]!.child.emit('exit', 0)
    await promise
  ...
  it('cleans up the temporary report directory even when a job fails', async () => {
    ...
    await tick()
    const reportDir = records[0]!.reportPath.substring(0, records[0]!.reportPath.lastIndexOf('/'))
    records[0]!.child.emit('exit', 2)
