 ...
 * Tests that the UMD bundles work in browser (global) and CommonJS contexts,
 * and run the session envelope end to end from the required exports: hello
 * exchange, seal, open, replay rejection, and a v4 key mismatch.
 *
...
/** Slice of a protocol bundle's exports used by these tests. */
interface ProtocolExports {
...
/** Slice of the v3 bundle's exports beyond `createProtocol`. */
interface V3Exports extends ProtocolExports {
  /** The v3 id and frame version byte. */
  V3: { id: string; version: number }
}

/** Slice of the v4 bundle's exports beyond `createProtocol`. */
interface V4Exports extends ProtocolExports {
...
const bundles = [
  ['v3', 'HyperfrontendNetworkProtocolV3', 3, (api: ProtocolExports) => api.createProtocol(silentLogger)],
  ['v4', 'HyperfrontendNetworkProtocolV4', 4, (api: ProtocolExports) => api.createProtocol(silentLogger, SHARED_KEY)],
] as const

/**
 * Requires a UMD bundle as a CommonJS module and returns its exports.
 *
 * @param version - The protocol whose bundle to load.
 * @returns The bundle's API.
 */
function requireApi(version: BundleVersion): ProtocolExports {
  return requireUmdBundle(loadBundleCode(getBundlePath(version, 'umd'))) as ProtocolExports
}
...
describe('@hyperfrontend/network-protocol UMD bundle', () => {
  it.each(bundles)('%s bundle file exists', (version: BundleVersion) => {
    expect(() => loadBundleCode(getBundlePath(version, 'umd'))).not.toThrow()
  })

  it.each(bundles)('%s minified bundle file exists', (version: BundleVersion) => {
    expect(() => loadBundleCode(getBundlePath(version, 'umd', true))).not.toThrow()
  })

  it.each(bundles)('%s bundle attaches %s to window global (browser mode)', (version: BundleVersion, globalName: string) => {
    expect(executeBundleInWindow(loadBundleCode(getBundlePath(version, 'umd')), globalName)).toBeDefined()
  })

  it.each(bundles)('%s bundle exports createProtocol function when required as CJS module', (version: BundleVersion) => {
    expect(typeof requireApi(version).createProtocol).toBe('function')
  })

  it.each(bundles)(
    '%s bundle emits a 99-byte plaintext hello carrying its version byte',
    async (version: BundleVersion, _globalName: string, versionByte: number, build: (api: ProtocolExports) => ProtocolProvider) => {
      const hello = await build(requireApi(version))(noop, noop, sessions(version).initiator).hello()
  ...
  it.each(bundles)(
    ...
    async (version: BundleVersion, _globalName: string, _versionByte: number, build: (api: ProtocolExports) => ProtocolProvider) => {
      const provider = build(requireApi(version))
  ...
  describe('V3 bundle', () => {
    it('exports the V3 definition', () => {
      expect((requireApi('v3') as V3Exports).V3).toEqual({ id: 'v3', version: 3 })
    })

    it('refuses a session negotiated for another protocol with invalid-session', async () => {
      const provider = requireApi('v3').createProtocol(silentLogger)
  ...
  describe('V4 bundle', () => {
    it('exports the V4 definition', () => {
      expect((requireApi('v4') as V4Exports).V4).toEqual({ id: 'v4', version: 4 })
    })

    it('exports a 16-character minimum shared key length', () => {
      expect((requireApi('v4') as V4Exports).MIN_SHARED_KEY_LENGTH).toBe(16)
    })

    it('validates a shared key against the minimum length', () => {
      const { isValidSharedKey } = requireApi('v4') as V4Exports
    ...
    it('throws when the shared key is shorter than 16 characters', () => {
      const { createProtocol } = requireApi('v4')
