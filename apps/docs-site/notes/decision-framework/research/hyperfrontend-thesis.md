...
- Costs that never go away even when the pattern fits: a second full document's memory and startup per feature; an independently hosted URL to build, serve, and keep alive; observability split across team boundaries; auth topology made explicit; routing/history and cross-boundary a11y still need design (article, "Collecting the complaints"); a sealed channel pays one key agreement per session (plus one password stretch for v4) and one authenticated-encryption operation per message in each direction (guides/compose-independent-features/guide.md §6); same-origin features share the host's thread so the watchdog cannot even run (guides/detect-unresponsive-feature/guide.md, Limits)
...
14. **Security envelope:** `protocol: 'none'` (local default) | `'v3'` | `'v4' + sharedKey` (16 characters or more); negotiated during the handshake; handshake frames stay plaintext, product messages travel sealed under per-session keys; negotiation is fail-closed, so a counterpart that cannot run the pinned protocol is denied (`reason: 'security-unavailable'`, surfaced as `error` on both sides) and a session the counterpart never confirms closes with `reason: 'security-unconfirmed'`; production builds opt in explicitly (`hf build --ci --allow-open` makes an open channel an acknowledged decision). Cost is one key agreement per session (plus one password stretch for v4) and one authenticated-encryption operation per message per direction. (ARCHITECTURE.md principle 5; README.md; guides/compose-independent-features/guide.md §6)
...
10. **Envelope cost is per session, not per message.** Key agreement (and, for v4, one password stretch) is paid once per session; each message then costs one authenticated-encryption operation per direction, and a frame the envelope rejects is reported as `error` with `reason: 'security-error'` rather than dropped silently. Label: documented behaviour. (guides/compose-independent-features/guide.md §6; README.md)
11. **Fail-closed negotiation.** A channel that pins a protocol opens on that protocol or not at all: a counterpart that cannot run it is denied with `reason: 'security-unavailable'`, both sides receive `error`, the host tears the mount down and the feature's `ready()` rejects; an open channel is only ever an explicit choice (`--allow-open`). Label: documented behaviour. (guides/embed-a-shipped-feature/guide.md §8; guides/compose-independent-features/guide.md §6)
12. **In-page script adversaries are outside the core trust model; crypto layers are defence-in-depth ("v4 is key-bound protection; v3 defeats only a script that can listen").** The article and security docs state this; a matrix comparing "security" across approaches must scope what is and is not defended. Label: scoped claim. (article, "Further security considerations"; docs-site `/docs/core-concepts/security`, source at `apps/docs-site/src/app/docs/core-concepts/security/`)
