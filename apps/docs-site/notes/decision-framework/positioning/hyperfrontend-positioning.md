...
| `pos.weakness.silent-plaintext-downgrade` | Withdrawn: negotiation is fail-closed, so a counterpart that cannot run the pinned protocol is denied with a runtime `error` on both sides | `security.channel-confidentiality`=c now conditions only on protocol choice and the in-page trust scope; thesis claim 11, documented behaviour |
| `pos.weakness.v1-envelope-collapse` | Withdrawn: the v3/v4 envelope pays key agreement once per session and one authenticated-encryption operation per message, and reports every rejected frame | thesis claim 10, documented behaviour |
...
| `pos.counterfactual.crypto-quantified` | Happened: v1 was replaced by the v3/v4 session envelope | `pos.weakness.v1-envelope-collapse` and `pos.weakness.silent-plaintext-downgrade` are withdrawn; `security.channel-confidentiality` stays Conditional on protocol choice and the in-page trust scope |
