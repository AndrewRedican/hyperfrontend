 ...
 * @returns The renderer the runtime loop drives: it draws this koi into the frame's own box, re-sizes to every pond the host announces, and on disposal destroys the Angular application that owns the scene, so a stood-down koi can be built again on wake.
 *
