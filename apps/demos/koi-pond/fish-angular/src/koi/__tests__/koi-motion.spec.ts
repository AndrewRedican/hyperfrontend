...
function swimmer(
  ...
  return createKoiMotion({
    profile: overrides.profile ?? koiProfile('angular'),
    pond: POND,
...
/**
 * Builds a koi bold enough to change depth rather than turn aside.
 *
 * @returns The koi's brain.
 */
...
describe('startle', () => {
  it('bolts from a strike that lands close', () => {
    expect(swimmer().startle({ x: 620, y: 410, intensity: 1 })).toBe(true)
  })

  it('ignores a strike far outside what it can notice', () => {
    expect(swimmer().startle({ x: 30_000, y: 30_000, intensity: 1 })).toBe(false)
  })

  it('ignores a strike too gentle to be worth fleeing', () => {
    expect(swimmer().startle({ x: 620, y: 410, intensity: 0.02 })).toBe(false)
  })
...
describe('depth', () => {
  ...
  it('forgets a pending request once the host grants a level itself', () => {
    const bold = diver()
    bold.observe([crossing()])
    run(bold, 0.1)
    bold.setDepth(5)
...
describe('outline', () => {
  it('reports itself by framework so the host can tell the shoal apart', () => {
    expect(swimmer().outline().framework).toBe('angular')
  })
  ...
  it('reports the course the host steers its neighbours around', () => {
    expect(swimmer({ heading: 1 }).outline().heading).toBeCloseTo(1)
  })
