...
describe('outbound wiring', () => {
  ...
  it('routes what the runtime emits onto the feature handle', () => {
    ...
    emit('outline', { framework: 'angular' })
    expect(harness.sends).toEqual([{ type: 'outline', data: { framework: 'angular' } }])
  })
})
...
describe('inbound wiring', () => {
  ...
  it('takes the identity the host assigned', () => {
    const harness = createHarness()
    harness.emit('identity', { framework: 'angular', seed: 401, url: 'https://pond.test/fish-angular/', depth: 2 })
    expect(harness.lastCall('adopt')).toEqual({ framework: 'angular', seed: 401, url: 'https://pond.test/fish-angular/', depth: 2 })
  })
