...
describe('outbound wiring', () => {
  ...
  it('routes what the runtime emits onto the feature handle', () => {
    ...
    emit('outline', { framework: 'react' })
    expect(harness.sends).toEqual([{ type: 'outline', data: { framework: 'react' } }])
  })
})
...
describe('inbound wiring', () => {
  ...
  it('takes the identity the host assigned', () => {
    const harness = createHarness()
    harness.emit('identity', { framework: 'react', seed: 401, url: 'https://pond.test/fish-react/', depth: 2 })
    expect(harness.lastCall('adopt')).toEqual({ framework: 'react', seed: 401, url: 'https://pond.test/fish-react/', depth: 2 })
  })
