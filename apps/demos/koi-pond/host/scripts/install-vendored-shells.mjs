#!/usr/bin/env node
/**
 * Installs every vendored koi shell tarball by explicit path.
 *
 * A bare `npm install` does not re-resolve a `file:` specifier whose path has
 * not changed. When a shell is repacked at the same version — the normal case
 * while a koi is being iterated on — npm reports "up to date", leaves the stale
 * integrity in `package-lock.json`, and installs the previous contents from
 * cache. On a cold cache the same drift fails hard with EINTEGRITY instead,
 * which is how it usually surfaces in CI.
 *
...
const VENDOR_DIR = join(HOST_ROOT, 'vendor')
