// context: Presentation-aware UI state for the feature, fed exclusively by the host protocol: the SDK's `presentation` announcement carries the display mode, and the session's `close` clears it. The app therefore never guesses its runtime from URLs, query params, or `window.parent` — opened directly in a tab nothing ever announces, the mode stays `null`, and dialog-only chrome simply never renders.
