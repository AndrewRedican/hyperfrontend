...
function swimmer(
  ...
  return createKoiMotion({
    profile: overrides.profile ?? koiProfile('svelte'),
    pond: POND,
...
describe('boundary behaviour', () => {
  it('turns back rather than leaving the pond when its seed obeys the correction', () => {
    ...
    // why: The svelte seed's first boundary approach obeys its correction, so this spec deterministically exercises the turn-back path; the slip arc is proven next with a seed that rolls it.
  ...
  it('sometimes slips out, sits away a while, and returns from the opposite side', () => {
    const bounds = pondBounds(POND)
    // why: The slip roll is seeded, and svelte's own first approach obeys — the vanilla seed's rolls a slip, so borrowing it drives the full leave-wait-warp-return arc deterministically.
...
describe('encounters', () => {
  it('gives way with its speed when the crossing allows it', () => {
    ...
    // why: The svelte koi is willing enough to settle a close crossing by changing level instead, which the depth specs prove; pinning that willingness low isolates the pace give-way here.
    const motion = swimmer({ profile: { ...koiProfile('svelte'), traits: { ...koiProfile('svelte').traits, depthWillingness: 0.2 } } })
    run(motion, 0.5)
...
describe('depth', () => {
  ...
  it('asks to change depth to settle a crossing when it is willing to', () => {
    const bold = createKoiMotion({
      profile: { ...koiProfile('svelte'), traits: { ...koiProfile('svelte').traits, depthWillingness: 0.95 } },
      pond: POND,
...
describe('outline', () => {
  it('reports itself by framework so the host can tell the shoal apart', () => {
    expect(swimmer().outline().framework).toBe('svelte')
  })
