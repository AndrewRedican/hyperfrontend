 ...
 * DOM and no runes, so it can be unit-tested frame by frame and so the Svelte
...
export function createKoiMotion(options: KoiMotionOptions): KoiMotion {
  ...
  const targetSpeed = (): number => {
    ...
    for (const neighbor of neighbors) {
      const resolution = resolveEncounter(encounterSelf(), neighbor, givesWay(profile.framework, neighbor.framework))
  ...
  return {
    ...
    setDepth(level) {
      ...
      transitioningUntilS = elapsed + 1.4
    },
