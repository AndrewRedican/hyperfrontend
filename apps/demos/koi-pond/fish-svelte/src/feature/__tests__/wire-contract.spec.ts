...
describe('outbound wiring', () => {
  ...
  it('routes what the runtime emits onto the feature handle', () => {
    ...
    emit('outline', { framework: 'svelte' })
    expect(harness.sends).toEqual([{ type: 'outline', data: { framework: 'svelte' } }])
  })
})
...
describe('inbound wiring', () => {
  ...
  it('takes the identity the host assigned', () => {
    const harness = createHarness()
    harness.emit('identity', { framework: 'svelte', seed: 3908, url: 'https://pond.test/fish-svelte/', depth: 2 })
    expect(harness.lastCall('adopt')).toEqual({ framework: 'svelte', seed: 3908, url: 'https://pond.test/fish-svelte/', depth: 2 })
  })
