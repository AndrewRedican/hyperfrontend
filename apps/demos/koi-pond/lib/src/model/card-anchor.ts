/**
 * Where a koi's identity card sits, and how that reaches CSS.
 *
 * Every koi renders its own card in its own framework's idiom, but the card
 * must park in the same place whoever draws it: riding off the koi's shoulder,
 * clamped inside the visible window. The maths lives here so every app shares
 * one set of margins and one clamp.
 */
import type { KoiState } from '../motion/koi-motion.js'
import type { PondEnvironment, Vec2 } from './types.js'
 ...
 *
 * @example Parking the card at a clamped anchor
 * ```typescript
 * card.style.transform = cardTransform(cardAnchor(motion.state, pond, size))
 * ```
 */
