  ...
  emitted: [
    {
      type: 'outline',
      description:
        "The koi's occupied outline as nose-first spine samples with a half-width each, plus heading, speed, depth, and behavioural phase. Ordinarily also the koi's current steering intent — travel, avoidance, or a depth pass, with the heading and turn gain it has committed to, the point it is steering toward, and the reach and clearance that bound the region it judges every crossing in — so the host can visualise the decision against the very terms the koi steers and perceives by; omitted while the koi is held. Ordinarily also the advancement the koi has committed to: at most twenty points along the path its wound manoeuvre carries it through, capped by the koi that produces it rather than by wire validation. While a visitor holds this koi, also the identity card's geometry — its frame and its three link rectangles — so the host can float real anchors and an inert shield over a card the pointer-transparent frame could never make interactive itself. High cadence, deliberately schema-less.",
    },
    {
      type: 'depth-request',
      description:
