...
function createHarness() {
   ...
   */
  const lastCall = (method: string): unknown => calls.filter((call) => call.method === method).pop()?.value
...
describe('outbound wiring', () => {
  ...
  it('routes what the runtime emits onto the feature handle', () => {
    ...
    emit('outline', { framework: 'vue' })
    expect(harness.sends).toEqual([{ type: 'outline', data: { framework: 'vue' } }])
  })
})
...
describe('inbound wiring', () => {
  ...
  it('takes the identity the host assigned', () => {
    const harness = createHarness()
    harness.emit('identity', { framework: 'vue', seed: 977, url: 'https://pond.test/fish-vue/', depth: 2 })
    expect(harness.lastCall('adopt')).toEqual({ framework: 'vue', seed: 977, url: 'https://pond.test/fish-vue/', depth: 2 })
  })
