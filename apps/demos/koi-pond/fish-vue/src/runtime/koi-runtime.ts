 ...
 * who opens `/fish-vue/` sees one koi swimming in clear water. That is the
...
export function createKoiRuntime(root: HTMLElement, buildRenderer: KoiRendererFactory = createKoiRenderer): KoiRuntime {
  ...
  const motion = createKoiMotion({ profile, pond, position: entry.position, heading: entry.heading, depth: ENTRY_DEPTH })
  ...
  const frame = (timestamp: number): void => {
    ...
    if (timestamp - lastOutlineAt >= OUTLINE_INTERVAL_MS) {
      ...
      const card = renderer.cardLinkRect()
