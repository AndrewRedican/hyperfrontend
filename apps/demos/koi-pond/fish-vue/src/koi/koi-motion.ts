 ...
 * DOM and no reactivity, so it can be unit-tested frame by frame and so the Vue
 * components next door stay the only browser-facing part of this app.
 *
...
export function createKoiMotion(options: KoiMotionOptions): KoiMotion {
  ...
  /**
   * What the encounter resolver needs to know about this koi right now.
   *
   * @returns This koi as the resolver sees it.
   */
  const encounterSelf = () => ({
    position,
    heading,
   ...
   */
  const encounter = (neighbor: NeighborObservation): EncounterResolution =>
