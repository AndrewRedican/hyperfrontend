...
function swimmer(
  ...
  return createKoiMotion({
    profile: overrides.profile ?? koiProfile('vue'),
    pond: POND,
...
describe('turning', () => {
  it('turns in bounded episodes rather than living in a turn', () => {
    ...
    // why: The wound helm carries the vue seed's longest boundary carve just over the turning threshold for a shade under six seconds, so the episode bound allows that while still refusing a koi that lives banked.
    expect(longestStreak / 60).toBeLessThan(6)
...
describe('boundary behaviour', () => {
  it('turns back rather than leaving the pond when its seed obeys the correction', () => {
    ...
    // why: The react seed's first boundary approach obeys its correction, so this spec deterministically exercises the turn-back path; under the gentler helm the vue seed's own approach stalls just outside the margin instead of coming about.
  ...
  it('sometimes slips out, sits away a while, and returns from the opposite side', () => {
    const bounds = pondBounds(POND)
    // why: The vue seed obeys its first correction, so this borrows the vanilla seed — whose first boundary approach rolls a slip — to drive the full leave-wait-warp-return arc deterministically.
...
describe('encounters', () => {
  it('gives way with its speed when the crossing allows it', () => {
    ...
    // why: Judged mid-encounter — under the bounded accelerations the vue koi clears this crossing astern within a second and a half and is then free to speed up again.
    run(motion, 1)
