...
describe('boundary behaviour', () => {
  it('turns back rather than leaving the pond when its seed obeys the correction', () => {
    ...
    // why: The vue seed's first boundary approach obeys its correction, so this fish's own profile deterministically exercises the turn-back path; the slip arc is proven below with a borrowed seed.
