...
const NEIGHBOR: NeighborObservation = { framework: 'vanilla', x: 10, y: 20, heading: 0.5, speed: 90, depth: 4, length: 120, girth: 14 }
...
describe('outbound wiring', () => {
  ...
  it('routes what the runtime emits onto the feature handle', () => {
    ...
    emit('outline', { framework: 'lit' })
    expect(harness.sends).toEqual([{ type: 'outline', data: { framework: 'lit' } }])
  })
})
...
describe('inbound wiring', () => {
  ...
  it('takes the identity the host assigned', () => {
    const harness = createHarness()
    harness.emit('identity', { framework: 'lit', seed: 6839, url: 'https://pond.test/fish-lit/', depth: 2 })
    expect(harness.lastCall('adopt')).toEqual({ framework: 'lit', seed: 6839, url: 'https://pond.test/fish-lit/', depth: 2 })
  })
...
describe('relayed neighbours', () => {
  ...
  it('empties the shoal when the relay sends something that is not a list', () => {
    const harness = createHarness()
    harness.emit('neighbors', { framework: 'vanilla' })
    expect(harness.lastCall('observe')).toEqual([])
  })
})
