...
import { KoiMotion } from '../koi-motion'

/** The pond every fixture here swims in. */
const POND: PondEnvironment = describePond(1200, 800, 1200, 800, false)

/** How this koi enters unless a fixture says otherwise. */
const MID_POND = { x: 600, y: 400 }
...
function swimmer(
  ...
  const profile = overrides.profile ?? koiProfile('lit')
  return new KoiMotion({
    profile:
      overrides.willingness === undefined
        ? profile
        : { ...profile, traits: { ...profile.traits, depthWillingness: overrides.willingness } },
    pond: POND,
    position: overrides.position ?? MID_POND,
...
describe('KoiMotion', () => {
  it('starts where it was placed', () => {
    expect(swimmer().state.position).toEqual(MID_POND)
  })
  ...
  it('swims forward under its own power', () => {
    ...
    run(motion, 1)
    expect(motion.state.position.x).toBeGreaterThan(MID_POND.x)
  })
  ...
  it('reports itself as the Lit koi', () => {
    expect(swimmer().profile.framework).toBe('lit')
  })
})
...
describe('boundary behaviour', () => {
  it('turns back rather than leaving the pond when its seed obeys the correction', () => {
    ...
    // why: Lit's own seed rolls a slip at its first boundary approach, so the react profile — whose first approach obeys its correction — is what deterministically exercises the turn-back path.
  ...
  it('sometimes slips out, sits away a while, and returns from the opposite side', () => {
    const bounds = pondBounds(POND)
    // why: The lit seed's first boundary approach rolls a slip, so this koi's own profile drives the full leave-wait-warp-return arc deterministically.
  ...
  it('curves away rather than reversing on the spot', () => {
    const bounds = pondBounds(POND)
    // why: The turn-back this measures needs a seed that obeys its correction — the react profile's does, where lit's own first approach slips out instead.
...
describe('encounters', () => {
  it('gives way with its speed when the crossing allows it', () => {
    ...
    // why: Lit's own willingness would settle this crossing a level down, and its brisk cruise overtakes a neighbour parked at 680 — so the fixture pins the depth and starts the neighbour further out to hold the pace path.
    const motion = swimmer({ willingness: 0.2 })
    run(motion, 0.5)
    const alone = motion.state.speed
    motion.observe([crossing({ x: 760, y: 402, heading: 0, speed: 40 })])
    run(motion, 1.5)
...
describe('depth', () => {
  ...
  it('asks to change depth to settle a crossing when it is willing to', () => {
    const bold = swimmer({ willingness: 0.95 })
...
describe('outline', () => {
  it('reports itself by framework so the host can tell the shoal apart', () => {
    expect(swimmer().outline().framework).toBe('lit')
  })
