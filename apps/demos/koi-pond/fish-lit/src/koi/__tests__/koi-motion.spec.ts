 ...
 */
function swimmer(
  overrides: { position?: { x: number; y: number }; heading?: number; depth?: number; willingness?: number } = {}
): KoiMotion {
...
function crossing(overrides: Partial<NeighborObservation> = {}): NeighborObservation {
  return { framework: 'react', x: 660, y: 460, heading: -Math.PI / 2, speed: 120, depth: 3, length: 130, ...overrides }
}
