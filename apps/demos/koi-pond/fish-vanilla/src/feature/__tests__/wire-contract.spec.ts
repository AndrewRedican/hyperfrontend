...
describe('outbound wiring', () => {
  ...
  it('routes what the runtime emits onto the feature handle', () => {
    ...
    emit('outline', { framework: 'vanilla' })
    expect(harness.sends).toEqual([{ type: 'outline', data: { framework: 'vanilla' } }])
  })
})
...
describe('inbound wiring', () => {
  ...
  it('takes the identity the host assigned', () => {
    const harness = createHarness()
    harness.emit('identity', { framework: 'vanilla', seed: 977, url: 'https://pond.test/fish-vanilla/', depth: 2 })
    expect(harness.lastCall('adopt')).toEqual({ framework: 'vanilla', seed: 977, url: 'https://pond.test/fish-vanilla/', depth: 2 })
  })
