...
function swimmer(
  ...
  return createKoiMotion({
    profile: overrides.profile ?? koiProfile('vanilla'),
    pond: POND,
...
describe('boundary behaviour', () => {
  it('turns back rather than leaving the pond when its seed obeys the correction', () => {
    ...
    // why: The react seed's first boundary approach obeys its correction, so this spec deterministically exercises the turn-back path; vanilla's own first approach is the slip the next spec proves.
  ...
  it('sometimes slips out, sits away a while, and returns from the opposite side', () => {
    const bounds = pondBounds(POND)
    // why: Vanilla's first boundary approach rolls a slip, so this drives the full leave-wait-warp-return arc deterministically.
...
describe('outline', () => {
  it('reports itself by framework so the host can tell the shoal apart', () => {
    expect(swimmer().outline().framework).toBe('vanilla')
  })
