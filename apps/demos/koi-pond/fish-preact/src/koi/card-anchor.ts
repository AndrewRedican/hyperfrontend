 ...
 * ```typescript
 * card.style.transform = cardTransform(cardAnchor(motion.state, pond.view, size))
 * ```
 */
export function cardAnchor(state: KoiState, view: PondWindow, size: CardSize): Vec2 {
  ...
  const x = head.x - view.x + state.length * CARD_AHEAD
  const y = head.y - view.y - state.length * CARD_ABOVE
  // why: A tapped fish near a window edge must still show its whole card — on touch there is no hover to chase it with.
  return {
    x: Math.min(Math.max(x, CARD_MARGIN), Math.max(CARD_MARGIN, view.width - size.width - CARD_MARGIN)),
    y: Math.min(Math.max(y, CARD_MARGIN), Math.max(CARD_MARGIN, view.height - size.height - CARD_MARGIN)),
  }
}
