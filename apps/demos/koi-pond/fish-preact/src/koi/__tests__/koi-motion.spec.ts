...
function swimmer(
  ...
  return createKoiMotion({
    profile: overrides.profile ?? koiProfile('preact'),
    pond: POND,
...
function diver(): KoiMotion {
  const profile = koiProfile('preact')
  return createKoiMotion({
...
describe('turning', () => {
  it('turns in bounded episodes rather than living in a turn', () => {
    ...
    // why: The preact seed's boundary-slip arc holds the gentler helm engaged just past four seconds once in ninety; five still proves a turn ends rather than becoming a state.
  ...
  it('cannot pair full escape speed with a tight turn', () => {
    ...
    // why: The preact seed's faint nerve runs its flee out within a second and a half, so full burst is sampled at one second while the bolt is still live.
    run(motion, 1)
...
describe('boundary behaviour', () => {
  it('turns back rather than leaving the pond when its seed obeys the correction', () => {
    ...
    // why: The react seed's first boundary approach obeys its correction, so this spec deterministically exercises the turn-back path; under the gentler helm the preact seed's own approach slips out instead of coming about.
  ...
  it('sometimes slips out, sits away a while, and returns from the opposite side', () => {
    const bounds = pondBounds(POND)
    // why: The vanilla profile's first boundary approach rolls a slip, which is what drives the full leave-wait-warp-return arc deterministically; the preact seed's own slip only comes after repeated corrections.
...
describe('encounters', () => {
  it('gives way with its speed when the crossing allows it', () => {
    ...
    // why: The preact koi is willing enough to settle a close crossing by changing level instead, which the depth specs prove; pinning that willingness low isolates the pace give-way here.
    const profile = koiProfile('preact')
    const motion = swimmer({ profile: { ...profile, traits: { ...profile.traits, depthWillingness: 0.2 } } })
    run(motion, 0.5)
    const alone = motion.state.speed
    // why: Preact's seeded itinerary has already banked this koi off the +x axis by now, so the slower neighbour is planted just ahead on its live course rather than at a fixed spot it has swum past.
    const { position, heading } = motion.state
    motion.observe([crossing({ x: position.x + Math.cos(heading) * 80, y: position.y + Math.sin(heading) * 80 + 2, heading, speed: 40 })])
    run(motion, 1.5)
  ...
  it('does not circle a neighbour it keeps observing', () => {
    ...
    // why: The old brain re-derived its evasion against its own moving heading every frame, which walked it through full circles; the anchored evasion must never accumulate one.
    expect(Math.abs(total)).toBeLessThan(Math.PI * 2)
  })
})
...
describe('outline', () => {
  it('reports itself by framework so the host can tell the shoal apart', () => {
    expect(swimmer().outline().framework).toBe('preact')
  })
