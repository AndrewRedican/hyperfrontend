...
describe('outbound wiring', () => {
  ...
  it('routes what the runtime emits onto the feature handle', () => {
    ...
    emit('outline', { framework: 'preact' })
    expect(harness.sends).toEqual([{ type: 'outline', data: { framework: 'preact' } }])
  })
})
...
describe('inbound wiring', () => {
  ...
  it('takes the identity the host assigned', () => {
    const harness = createHarness()
    harness.emit('identity', { framework: 'preact', seed: 5862, url: 'https://pond.test/fish-preact/', depth: 2 })
    expect(harness.lastCall('adopt')).toEqual({ framework: 'preact', seed: 5862, url: 'https://pond.test/fish-preact/', depth: 2 })
  })
