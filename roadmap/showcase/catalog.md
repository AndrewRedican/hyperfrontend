...
| **Prereq**     | **deploy** ([strategy](00-strategy.md#deployment-and-the-origin-boundary-layer)) · `06` generator — every build-out demo implicitly needs `06`                                                                                                                                                               |
| **Frameworks** | React / Vue / Vanilla / **Svelte / Solid / Preact / Lit** are live; **Angular** deferred                                                                                                                                                                                                                     |

---

## 1. Must-ship core (P0) — the ten the portfolio leads with

The cut line. These ten, _together_, prove the whole thesis: all four tiers, all five topologies, the framework palette, serious **and** playful, one real backend, and the centerpiece that composes them. Everything below §1 broadens range but proves nothing the core doesn't already. Rows #1 (Clock), #2 (Heartbeat), and #8 (Koi pond) have shipped and left the backlog; their numbers stay reserved so cross-references hold.
...
**Coverage check** (why exactly these ten, counting the three shipped):
...
- **Frameworks** — React, Vue, Vanilla, Svelte, Solid, Preact, and Lit shipped; Web Components on colourcopia. ✔ palette shown, not claimed.
...
- **Prereq spread** — 06 underwrites everything still in the backlog. ✔
...
Four on-disk reservations remain, each a single `demo-<name>` app (`@hyperfrontend/demo-<name>`, no source). The composition model is settled: **feature-only demos are hosted by the docs-site gallery** (a slot stays a single self-contained feature app); demos whose point requires their own host grow host sub-apps under the slot directory. New slots are scaffolded by the generator ([06](06-demo-2-and-generator.md)); the hand-built `koi-pond` slot is live.
...
New slots the core/§2 introduce: `stock`, `auth`, `nested` (re-uses `events`), `colourcopia`, `flagship`, `payments`, `bounty`, `terminal`, `voice`, `json-fixer`, `drag`, `chat`, `whiteboard`. All created under the host+hostee model — never as bare single-app placeholders.

---

## 6. Build order (locked)

1. **[06](06-demo-2-and-generator.md) generator** — proven by reproducing the heartbeat demo's shape; unblocks cheap repeatable demos.
2. **Breadth, parallelized:** [08](08-breadth-boundary-respecting.md) (Stock, Auth, Payments, gallery-features) ∥ [09](09-breadth-pattern.md) (Russian-doll, Framework chess, chat/whiteboard) ∥ [10](10-breadth-spectacle-plugin.md) (Colourcopia, Drag, Terminal, Voice) — build the rest of P0, then P1, then P2 as budget allows.
3. **[11](11-flagship-composed-app.md) Flagship** — composes the core as its windows (Omni-bar, Navbar, Footer, Terminal become its furniture).
4. **[12](12-gallery-docs-integration.md) gallery residue** — every shipped demo registered live as it lands (invariant #3), plus the per-demo how-to guides.
