...
**Depends on** all prior plans · **Type** D+E · **Status**: **Executed** (2026-08-01) — `features@0.2.0` published 2026-07-05, `0.3.0` published 2026-08-01, and the [post-publish checklist](#after-publishing-the-execution-checklist) ran against 0.3.0 the same day: published artifacts verified cold (steps 1–2), every demo workaround deleted (step 3), the shell regenerated by a real `npx hf build` (protocol `v1`, declared in the feature config) with the interim pack executor retired (step 4), the docs-site embed re-seated on shell events with a static import and the self-contained 0.2.0 tarball vendored (step 5). Step 1's scratch consumer surfaced two fresh `hf build` packaging blockers plus two API-surface gaps — fixed and shipped in `features@0.4.0`/`builder@0.1.3` (published 2026-08-02); on 2026-08-03 the fixes were re-verified cold against a fresh install and the demo dropped its consumer-lens workarounds and re-vendored the shell on 0.4.0. Remaining: the [docs refresh](#after-publishing-the-execution-checklist) (step 8) and the user-side deploys (step 6 phase 6: Railway rebuild, docs-site deploy).
...
6. **Re-verify the demo phases** (Demo 1, plan 04 — delivered): Phase 1 (dev loop with the shipped debug UI), Phase 3 (drive every accepted action, observe every emitted action — the messaging thesis, live for the first time), Phase 4 (tarball proof, now with typed `createFeatureShell`), Phase 5 (docs-site build + both embeds). Phase 6 stays user-side: create the Railway static service, attach the domain, set `NEXT_PUBLIC_CLOCK_FEATURE_URL`, verify the live cross-site embed with a clean message log.
