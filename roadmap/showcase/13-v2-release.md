...
The release loop ran (features is published through 0.5.x and all three live demos consume it). What remains is the docs refresh the loop owed, one hygiene republish that never happened, and verifying the live embeds after the next redeploy.
...
3. **Verify the live embeds after the next merge to `main`**: Railway redeploys the demo services via the GitHub integration once CI passes; the deployed clock currently refuses the handshake until that rebuild. Confirm every gallery embed goes live with clean message logs. For the koi pond specifically: confirm the `hyperfrontend-demo-koi-pond` service serves the composed site (host at `/`, fish at `/fish-<framework>/`, no SPA rewrite), and watch the full shoal plus the card → expanded transition on real GPU hardware — local verification runs on software rendering, which throttles the simulation and can drop inner sessions in ways real devices should not.
