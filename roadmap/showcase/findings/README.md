...
| F-017 | [A COOP-isolated feature origin breaks the windowed display modes silently, hanging the host to open-timeout](017-coop-isolated-origin-breaks-popup-mode-silently.md)               | confusing-error | medium   | demo-koi-pond     | open    | —              |
...
| F-025 | [Bundled CJS deps' ESM wrappers still reference `__dirname` and `require`, so four builder ESM entries throw on import](025-bundled-cjs-deps-esm-wrappers-reference-cjs-globals.md) | other           | high     | jest-to-node-test | open    | —              |
