...
- **Embedding.** Each service ships `public/serve.json` (pond + all seven fish) carrying `frame-ancestors`, `trailingSlash: true` and `directoryListing: false`. `frame-ancestors` is checked against EVERY ancestor, so the koi value must name the pond AND the docs site (chain is docs-site -> pond -> koi); naming only the pond blanks the shoal. In the pond's file the `fish-*/**` block comes second — serve merges matches in array order, later wins.
